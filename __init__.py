...
def reemplazar_signos(text):
    ...
    def reemplazar_dobles(text):
        ...
        for match in matches_doble:
            ...
            if counter % 2 != 0:
                replacement = r'[latex]\\begin{math}'
            else:
                replacement = r'\\end{math}[/latex]'
    ...
    if '[$]' in text or '[/$]' in text:
        ...
        return text, False
    # Reemplazar cualquier [latex]\begin{math por [latex]\begin{math}
    text = re.sub(r'\s*\[latex\]\\begin{math', r'[latex] \\begin{math}', text, flags=re.MULTILINE)
